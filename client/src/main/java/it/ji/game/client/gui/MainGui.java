...
public class MainGui extends javax.swing.JFrame implements ClientListener {
    ...
    public void positionChanged(String username, Coordinates coordinates) {
        LoggerG.setMessage("[DEBUG][EVENT] Position changed: "+username+" to "+coordinates).system().system();
    ...
    public void serverIsAlive(boolean isAlive, Optional<String> serverID) {
        /*LoggerG.setMessage("[DEBUG][EVENT] Server is alive: "+isAlive).system();
        LoggerG.setMessage("[WARNING] server alive status not yet managed on Main Gui").system();*/
    ...
    public void movePlayerToDirection(PlayerType playerType, Coordinates direction){
        LoggerG.setMessage("[DEBUG][EVENT] Moving player to direction: "+direction).system().system();
    ...
    public void moveUp(PlayerType playerType) throws ArrayIndexOutOfBoundsException{
        LoggerG.setMessage("[DEBUG][EVENT] Moving up").system().system();
        Player player = ClientGameManager.getInstance().getPlayerFromType(playerType);
    ...
    private Direction getDirectionFromDelta(Coordinates delta) {
        if (delta.x() == 0 && delta.y() == 0){
            LoggerG.setMessage("NONE").system();
            return Direction.NONE;
        }
        if (delta.x() == 0 && delta.y() == -1){
            LoggerG.setMessage("UP").system();
            return Direction.UP;
        }
        if (delta.x() == 0 && delta.y() == 1){
            LoggerG.setMessage("DOWN").system();
            return Direction.DOWN;
        }
        if (delta.x() == 1 && delta.y() == 0){
            LoggerG.setMessage("RIGHT").system();
            return Direction.RIGHT;
        }
        if (delta.x() == -1 && delta.y() == 0){
            LoggerG.setMessage("LEFT").system();
    ...
    private void formKeyPressed(java.awt.event.KeyEvent evt) {
        ...
        LoggerG.setMessage("Key pressed: "+evt.getKeyCode()).system();
        int keyCode = evt.getKeyCode();
        LoggerG.setMessage("Key code: "+keyCode).system();
